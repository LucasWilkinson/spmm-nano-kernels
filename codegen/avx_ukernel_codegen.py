...
def microkernel_id(arch, acc_dims, nanokernels):
    _hash = hashlib.md5(" ".join([str(p) for p in nanokernels]).encode("utf8")).hexdigest()
    return f'{nanokernel_hash(nanokernels)}_{arch}_{acc_dims[0]}x{acc_dims[1]}'


def microkernel_typename(scalar, arch, acc_dims, nanokernels):
    return f'MicroKernel_{scalar}_{microkernel_id(arch, acc_dims, nanokernels)}'
...
def ukernel_codegen(acc_dims, nanokernels,
    ...
    micro_kernel_typename_names = [
        microkernel_typename(scalar, arch, acc_dims, nanokernels)
        for scalar, arch in vec_configs
    ]
    ...
    def ukern_header_path(microkernel_typename):
        return include_output_root + microkernel_typename + '.h'
    ...
    if build_factories_for is not None:
        ...
        for (scalar, arch), micro_kernel_typename in zip(vec_configs, micro_kernel_typename_names):
            ...
            factory_output_root = f'{output_root}/{arch}/factories/{nanokernel_hash_}/'
            os.makedirs(include_output_root, exist_ok=True)
            ...
            microkernel_id_ = microkernel_id(arch, acc_dims, nanokernels)

            header = "/".join(ukern_header_path(micro_kernel_typename).split('/')[-2:])
            ...
            with open(factory_output_root + f'packer_{scalar}_{arch}_nr_{acc_dims[1]}.cpp', 'w+') as f:
            ...
            for kernel_desc in build_factories_for:
                ...
                with open(factory_output_root + f'executor_{kernel_desc}_{scalar}_{arch}_nr_{acc_dims[1]}.cpp', 'w+') as f:
                    f.write(f'#ifdef {min_instruction_sets[reg_width_bits]}\n')
    ...
    for (scalar, arch), micro_kernel_typename in zip(vec_configs, micro_kernel_typename_names):
        ...
        with open(ukern_header_path(micro_kernel_typename), 'w+') as f:
            ...
            f.write(f'    static constexpr int vec_width_bits = {reg_width_bits};\n')
    ...
    __ALWAYS_INLINE static void _microkernel_{acc_width_str}(
        ...
        const bool load_c)
    {{\n{gen_executor_body(scalar, reg_width_bits, [acc_dims[0], acc_width], max_acc_width, nanokernels, 
    ...
    __ALWAYS_INLINE static void _microkernel_masked_1(
        ...
        Mask last_reg_mask,
        const bool load_c)
    {{\n{gen_executor_body(scalar, reg_width_bits, [acc_dims[0], 1], max_acc_width, nanokernels,
